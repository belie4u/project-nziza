...
from django.forms import MultiWidget, FileInput, TextInput
...
def _option_image_field(form, product, option):
...
def _option_file_field(form, product, option):
    return forms.FileField(
        label=option.name,
        required=option.required,
        widget=forms.FileInput(),  # You can customize this widget if needed
        help_text=option.help_text,
    )
...
class LocationWidget(MultiWidget):
    def __init__(self, choices=None, attrs=None):
        # Initialize widgets: a CheckboxSelectMultiple for choices, followed by FileInputs and TextInputs for each choice
...
class LocationField(forms.MultiValueField):
    def __init__(self, *args, choices=None, **kwargs):
        # Define fields: MultipleChoiceField for checkboxes, ImageFields and CharFields for each choice
...
def _option_location_field(form, product, option):
    return LocationField(
        label=option.name,
        required=False,
        widget=LocationWidget(choices=option.get_choices()),
        help_text=option.help_text,
        choices=option.get_choices(),
    )
...
class AddToBasketForm(forms.Form):
    OPTION_FIELD_FACTORIES = {
        ...
        Option.IMAGE: _option_image_field,
    ...
    def __init__(self, basket, product, *args, data=None, **kwargs):
        # Set up the prefix and get instance from kwargs
        self.prefix = kwargs.get("prefix", None)
        instance = kwargs.get("instance", None)

        # If no instance is passed, set initial display_order dynamically
        if not instance:
            initial = {"display_order": self.get_display_order()}
            initial.update(kwargs.get("initial", {}))
            kwargs["initial"] = initial

        # Call the parent constructor to finish initialization
        super().__init__(data, *args, **kwargs)

        # Existing setup logic for basket and product
        self.basket = basket
    ...
    def get_display_order(self):
        """
        Extracts the display order from the form's prefix.
        Assumes that the prefix follows a format like 'product-1', 'product-2', etc.
        """
        if self.prefix:
            try:
        ...
        return 0  # Default display order if no prefix is provided

    # Dynamic form building methods
    def _create_parent_product_fields(self, product):
        """
        Adds the fields for a "group"-type product (e.g., a parent product with a
        list of children).
        """
        choices = []
        disabled_values = []
        for child in product.children.public():
            # Build a description of the child, including any pertinent attributes
    ...
    def _add_option_field(self, product, option):
        ...
        """
        # Check if the option type is IMAGE or LOCATIONS
        if option.type == Option.IMAGE:
            ...
            # Use the appropriate field factory from OPTION_FIELD_FACTORIES
    ...
    def clean_child_id(self):
        ...
        self.child_product = child  # Cache the loaded child product
        return self.cleaned_data["child_id"]

    def clean_quantity(self):
        qty = self.cleaned_data.get("quantity")

        # If quantity is None (user left it empty), set it to the initial value
        if qty is None:
            qty = self.fields["quantity"].initial
        ...
        if basket_threshold:
            ...
            if qty > max_allowed:
                raise forms.ValidationError(
                    _("Due to technical limitations, we cannot ship more than %(threshold)d items in one order. Your basket currently has %(basket)d items.")
    ...
    def clean(self):
        """
        Custom clean method to validate the product's price, currency, quantity, 
        and handle uploaded files for JSON serialization.
        """
        info = self.basket.strategy.fetch_for_product(self.product)

        # Validate that the product has a price
        if not info.price.exists:
            raise forms.ValidationError(
                _("This product cannot be added to the basket because a price could not be determined for it.")
            )

        # Validate currency consistency in the basket
        if self.basket.currency and info.price.currency != self.basket.currency:
            raise forms.ValidationError(
                _("This product cannot be added to the basket as its currency differs from other products in your basket.")
            )

        # Check if the desired quantity is allowed
        ...
        # Process uploaded files for options
        for option in self.parent_product.options:
            if option.code in self.cleaned_data:
                # Handle IMAGE option type
                if option.type == Option.IMAGE:
                    image_file = self.cleaned_data[option.code]
                    if image_file:
                        file_path = self._handle_uploaded_file(image_file)
                        # Convert to file path
                        self.cleaned_data[option.code] = file_path

                # Handle LOCATIONS option type with image
                elif option.type == Option.LOCATIONS:
                    location_data = self.cleaned_data[option.code]
                    if location_data and 'image' in location_data:
                        image_file = location_data['image']
                        if image_file:
                            file_path = self._handle_uploaded_file(image_file)
                            # Convert to file path
                            location_data['image'] = file_path
                            # Ensure the cleaned data is updated
                            self.cleaned_data[option.code] = location_data
    ...
    def cleaned_options(self):
        ...
        for option in self.parent_product.options:
            if option.code in self.cleaned_data:
                ...
                if value is False:
                    continue
                if isinstance(option.type, MultiWidget):
                    clean_values = [v for v in value if v not in EMPTY_VALUES]
                    if option.required or clean_values:
                        options.append(
                            {"option": option, "value": clean_values})
                else:
...
class SimpleAddToBasketForm(SimpleAddToBasketMixin, AddToBasketForm, LocationWidget):
    """
